{{-- user-blocked__window_active --}}
<div class="user-blocked__window">
    <div class="user-blocked__content">
        <div class="user-blocked__content-icon"></div>
        <div class="user-blocked__content-wr-text">
            <p class="user-blocked__content-text">Данная учетная запись <br> заблокирована.</p>                
            <p class="user-blocked__content-text">Для связи с менеджером напишите: <br> yesokolova@alephtrade.com</p>                              
        </div>
    </div> 
    <div class="user-blocked__close">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 30" fill="none">
            <path d="M1.76465 1.76465L26.7214 26.7214" stroke="white" stroke-width="3" stroke-linecap="round"/>
            <path d="M26.4705 1.76465L1.76465 26.4705" stroke="white" stroke-width="3" stroke-linecap="round"/>
        </svg>
    </div>
</div>